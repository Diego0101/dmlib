...
class ZernikeControl:
    ...
    def __init__(self, dm, calib, pars={}, h5f=None):
        ...
        self.h5_make_empty('flat_on', (1, ), bool)
...
class SVDControl(ZernikeControl):
    ...
    def __init__(self, dm, calib, pars, h5=None):
        ...
        if enabled:
            ...
            svd_modes = 0
        ignore = np.array(self.svd_pars['zernike_exclude'], dtype=np.int32)
        nignore = ignore.size
        ...
        if self.P:
            ...
            H = self.calib.H
        smap = np.zeros(H.shape[0], dtype=bool)
        smap[ignore - 1] = 1
        O1 = np.eye(H.shape[0])
...
def get_noll_indices(pars):
    noll_min = np.array(pars['min'], dtype=np.int32)
    noll_max = np.array(pars['max'], dtype=np.int32)
    minclude = np.array(pars['include'], dtype=np.int32)
    mexclude = np.array(pars['exclude'], dtype=np.int32)

    mrange = np.arange(noll_min, noll_max + 1, dtype=np.int32)
    ...
    zernike_indices = np.array(zernike_indices, dtype=np.int32)

    log = logging.getLogger('ZernikeControl')
    log.info(f'selected Zernikes {zernike_indices}')

    assert (zernike_indices.dtype == np.int32)
